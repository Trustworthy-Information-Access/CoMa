...
# Fill in here with eval outputs paths
checkpoint_paths = [
...
datasets = [
    ...
    "OK-VQA", "A-OKVQA", "DocVQA", "InfographicsVQA", "ChartQA", "Visual7W", "ScienceQA", "VizWiz", "GQA", "TextVQA", "**VQA**",
    "VisDial", "CIRR", "VisualNews_t2i", "VisualNews_i2t", "MSCOCO_t2i", "MSCOCO_i2t", "NIGHTS", "WebQA", "FashionIQ", "Wiki-SS-NQ", "OVEN", "EDIS", "**Retrieval**",
    "MSCOCO", "RefCOCO", "RefCOCO-Matching", "Visual7W-Pointing", "**VG**",
]

image_tasks = ["**Classification**", "**VQA**", "**Retrieval**", "**VG**"]
...
for checkpoint_path in checkpoint_paths:
    ...
    for dataset in datasets:
        if dataset in image_tasks:
            checkpoint_scores[dataset] = sum(task_scores) / len(task_scores) if task_scores else "N/A"  # Average score for image tasks
            task_scores = []
            continue
        ...
        if os.path.isfile(score_file):
            ...
            task_scores.append(score)  # Collect scores for averaging if needed
            all_scores.append(score)  # Collect all scores for final output
        else:
            checkpoint_scores[dataset] = "N/A"  # If no score file, set to 'N/A'
    checkpoint_scores["Final Score"] = sum(all_scores) / len(all_scores) if all_scores else "N/A"  # Average of all scores
...
for dataset in datasets + ["Final Score"]:
    row = []
    for experiment, scores in gathered_scores_by_exp.items():
