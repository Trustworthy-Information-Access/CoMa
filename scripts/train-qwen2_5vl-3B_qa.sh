...
if [ -z $EXP_NAME ]; then
    export EXP_NAME=qwen2_5.3B.CoMA.qa.BS${BS}.lr5e5.C${TOTAL_COMPRESS_TOKENS}.epoch1warm0_05
fi
...
cmd="torchrun --nnodes=$N_NODES --nproc_per_node=$N_GPUS --node_rank=$NODE_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT --max_restarts=0 train.py \
    ...
    --model_name Qwen/Qwen2.5-VL-3B-Instruct --model_type qwen2_5_vl_compression \
