...
if [ -z "$EXP_NAME" ]; then
    export EXP_NAME=qwen2_5.7B.CoMA.retrv.lora16.avg.BSr${BSr}:64.lr5e5.C${TOTAL_COMPRESS_TOKENS}.steps1500
fi
...
if [ -z "$MODEL_PATH" ]; then
    export MODEL_PATH=$OUTPUT_DIR/qwen2_5.7B.CoMA.qa.BS256.lr5e5.C32.epoch1warm0_05/merged
fi
...
cmd="torchrun --nnodes=$N_NODES --nproc_per_node=$N_GPUS --master_addr=$MASTER_ADDR --node_rank=$NODE_RANK --master_port=$MASTER_PORT --max_restarts=0 train.py \
    ...
    --grad_cache --gc_q_chunk_size 4 --gc_p_chunk_size 4 \
