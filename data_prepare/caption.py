...
openai_api_base = "http://localhost:15100/v1"  # 
model_name = '..t/Qwen2.5-VL-3B-Instruct'
...
def description_selection():
    descriptions = [
        'Please describe this image in one sentence.',
        'Please generate a short description for this image.',
        'Please describe the main content of the image in concise language.',
        'Please write a brief caption for this image.',
        'Please summarize the content of this image in one sentence.',
        'Please generate a brief image description that highlights the core subject of the image.',
        'Please briefly describe the scene in the image.'
    ]
...
def request_single(row):
    ...
    questions = description_selection()
...
if __name__ == '__main__':
    ...
    completed_files = [item for item in os.listdir(store_dir) if '.json' in item]
    files = list(set(files) - set(completed_files))
    files = [os.path.join(source_dir, item) for item in files]
