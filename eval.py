...
def main():
    ...
    dataset_configs = {}
    for dataset_config in data_args.dataset_config:
        with open(dataset_config, 'r') as yaml_file:
            dataset_configs |= yaml.safe_load(yaml_file)

    # --- Main Evaluation Loop ---
    for dataset_idx, (dataset_name, task_config) in enumerate(tqdm(dataset_configs.items(), desc="Main Evaluation Loop")):
        # 0. load dataset
        if dist.is_initialized():
            dist.barrier()
