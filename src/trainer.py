...
class MMEBTrainer(Trainer):
    def __init__(self, *args, model_args=None, train_dataloader=None, **kwargs):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if self.model.projector is not None:
            torch.save(self.model.projector.state_dict(), os.path.join(output_dir, 'projector.pth'))
...
class GradCacheLateProcessTrainer(MMEBTrainer):
    ...
    def training_step(self, model, inputs, *args, **kwargs) -> torch.Tensor:
        model.train()
        queries, targets, negatives = inputs["qry"], inputs["tgt"], inputs.get("neg", None)
        ...
        if negatives is not None:
            ...
            model_inputs = {'qry': queries}, {'tgt': targets}, {'neg': negatives }
        else:
            model_inputs = {'qry': queries}, {'tgt': targets}
        ...
        if _distributed:
            self.gc.models = [model for _ in range(len(model_inputs))]
            loss = self.gc(*model_inputs, no_sync_except_last=_distributed, deepspeed=deepspeed)
        else:
...
class OpenQATrainer(Trainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if state_dict is None:
            state_dict = self.model.state_dict()
        prefix = 'encoder.'
        encoder_modules = [k.startswith(prefix) for k in state_dict.keys() if not k.startswith('ref_model.')]
