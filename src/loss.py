...
class SimpleContrastiveLoss:
    ...
    def __call__(self, x: Tensor, y: Tensor, z: Tensor = None, target: Tensor = None, reduction: str = 'mean') -> Tensor:
        if target is None:
        ...
        if z is not None:
            bsz, dim = x.shape
            # (bsz, 1, dim) * (bsz, dim, neg_num) -> (bsz, 1, neg_num)
            neg_logits = torch.matmul(x.unsqueeze(1), z.view(bsz, -1, dim).transpose(1, 2))
            # (bsz, neg_num)
