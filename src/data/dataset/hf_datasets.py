...
def _interleave_iterable_datasets(
    ...
    for idx, d in enumerate(datasets):
        print_master(f"\t\tDataset {idx} features: {[f for f in datasets[idx].features]}")
    print_master("=" * 50)
    ...
    rows_nums = [d.num_rows for d in datasets]
    ...
    # Return new daset
    dataset = IterableDataset(ex_iterable=ex_iterable, info=info, split=split, token_per_repo_id=token_per_repo_id)
    # TODO
    setattr(dataset, 'num_rows', sum(rows_nums))
    return dataset

class NoInterleaveMultiSourcesBatchesIterable(_BaseExamplesIterable):
    def __init__(
        self,
    ...
    def shuffle_data_sources(self, generator: np.random.Generator) -> "NoInterleaveMultiSourcesBatchesIterable":
        """Shuffle each underlying examples iterable."""
        ex_iterables = [ex_iterable.shuffle_data_sources(generator) for ex_iterable in self.ex_iterables]
        return NoInterleaveMultiSourcesBatchesIterable(ex_iterables)

    def __iter__(self):
        iterators = [_HasNextIterator(ex_iterable) for ex_iterable in self.ex_iterables]
        indices_iterator = range(len(iterators))
    ...
    ) -> "NoInterleaveMultiSourcesBatchesIterable":
        """Either keep only the requested shard, or propagate the request to the underlying iterable."""
        return NoInterleaveMultiSourcesBatchesIterable(
