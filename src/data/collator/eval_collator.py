...
class MultimodalEvalDataCollator:
    ...
    def _get_batch_inputs(self, batch, text_keyname, image_keyname):
        texts, visual_inputs, image_paths = [], [], []
        ...
        inputs = {'text': texts, 'images': visual_inputs, 'image_paths': image_paths}
        return inputs
    ...
    def __call__(self, examples):
        ...
        try:
            ...
            if not all("cand_name" in info for info in dataset_infos):
                dataset_infos = [info | {"cand_name": info["cand_names"][0]} for info in dataset_infos]
        except Exception as e:
            print(e)
            if torch.distributed.get_rank() == 0:
                breakpoint()
            torch.distributed.barrier()
        image_paths = inputs.pop('image_paths')
        dataset_infos = [info | {"image_paths": paths} for info, paths in zip(dataset_infos, image_paths)]
