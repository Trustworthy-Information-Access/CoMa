...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for qry_text, qry_image_path, cand_id, retrieval_type in \
        ...
        if (not qry_text and not qry_image_path):
            print("empty inputs")
            continue
        qry_text = qry_text.replace("<|image_1|>", VLM_IMAGE_TOKENS[model_backbone])
...
@AutoEvalPairDataset.register("gui")
def load_gui_dataset(model_args, data_args, training_args, *args, **kwargs):
    dataset_name = kwargs.get("dataset_name", "gui")
    ...
    kwargs['global_dataset_name'] = f'gui/{subset_name}'
