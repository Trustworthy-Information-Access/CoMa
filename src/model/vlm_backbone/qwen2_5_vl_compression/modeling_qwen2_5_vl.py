...
from src.utils import print_rank, visual_attention_masks
...
class Qwen2_5_VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        attn_implementation = "flash_attention_2"  # ! force flash attention 2 for now
...
def visual_attention_masks(mask_list: List[torch.Tensor], name):
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    def __init__(self, config: Qwen2_5_VLConfig):
        ...
        self.split_layer = config.split_layer if hasattr(config, "split_layer") else -1
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        sep_indices: Optional[torch.LongTensor] = None,
        *args, **kwargs
        ...
        assert causal_mask is not None and causal_mask.dim() == 4, "4D attention mask must be provided."

        retrv_flag = False
        is_generating = False
        if sep_indices is not None:
            is_generating = causal_mask.shape[-2] == 1 # HACK: if model is generating autoregressively during inference
            if is_generating and self.split_layer >= 0:
                min_type = torch.finfo(inputs_embeds.dtype).min
                for batch_idx in range(len(causal_mask)):
                    sep1 = sep_indices[batch_idx][0]
                    causal_mask[batch_idx, :, :, :sep1] = min_type  # A only see C, Q's KVCache
                sep_indices = None  # ignore it in layer forward
            else:
                # full attention for image and compression tokens
                # new_causal_mask = causal_mask.clone()
                # for batch_idx in range(len(new_causal_mask)):
                #     sep1, sep2, sep3 = sep_indices[batch_idx][:3]
                #     new_causal_mask[batch_idx, :, :, :sep1] = 0
                #     new_causal_mask[batch_idx, :, sep1:sep2, sep1:sep2] = 0
                #     new_causal_mask[batch_idx, :, sep2:sep3, sep2:sep3] = 0
                # causal_mask = new_causal_mask
                retrv_flag = sep_indices[0, -1] == 0  # HACK: if is retrieval task
                if retrv_flag:
                    assert all(sep_indices[:, -1] == 0), "All batch items must for retrieval task."
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            # we break in the middle of the layers, and use its hidden compression states for retrieval
            # if layer_idx == self.split_layer and retrv_flag:
            #     break

            if output_hidden_states:
                all_hidden_states += (hidden_states,)
            
            # if sep_indices is not None and not retrv_flag:
            if sep_indices is not None and not retrv_flag and not is_generating:
                causal_mask = self._update_causal_mask_by_sep_indices(
                    causal_mask, sep_indices, layer_idx, inputs_embeds.dtype)  # update for qa task only
            ...
            #         # TODO: mean pooling only
            #         embeddings = torch.mean(embeddings, dim=1)
            #         # return torch.nn.functional.normalize(embeddings, p=2, dim=-1)
            #         return embeddings

                # cls_embeddings_lst = []
                # for batch_idx in range(len(sep_indices)):
                #     sep1, sep2, sep3 = sep_indices[batch_idx][:3]
                #     if sep3 != sep2:
                #         assert sep3 - sep2 == 32, sep3 - sep2
                #         # (1, hidden_sizze)
                #         cls_embeddings = pooling(hidden_states[batch_idx:batch_idx+1, sep2:sep3, :])
                #         cls_embeddings = cls_embeddings.unsqueeze(1).repeat(1, hidden_states.size(1) - sep3, 1)
                #         hidden_states[batch_idx:batch_idx+1, sep3:, :] += cls_embeddings
                #         # cls_embeddings_lst.append(cls_embeddings)
                #     else:
                #         # raise ValueError("sep3 - sep2 != 32")
                #         assert sep2 - sep1 == 32, sep2 - sep1
                #         cls_embeddings = pooling(hidden_states[batch_idx:batch_idx+1, sep1:sep2, :])
                #         cls_embeddings = cls_embeddings.unsqueeze(1).repeat(1, hidden_states.size(1) - sep1, 1)
                #         hidden_states[batch_idx:batch_idx+1, sep1:, :] += cls_embeddings

        # visual_attention_masks(causal_mask_list, name='ret' if retrv_flag else 'qa')
        ...
        #         import os
        #         if sep3 != sep2:
        #             assert sep3 - sep2 == int(os.environ.get('TOTAL_COMPRESS_TOKENS', 0)), f"{sep3 - sep2} != {os.environ.get('TOTAL_COMPRESS_TOKENS', 0)}"
        #             # (1, hidden_size)
        #             cls_embeddings = pooling(hidden_states[batch_idx:batch_idx+1, sep2:sep3, :])
        #             cls_embeddings_lst.append(cls_embeddings)
        #         # else:
        #         #     raise ValueError(f"{sep3 - sep2} != {os.environ.get('TOTAL_COMPRESS_TOKENS', 0)}")
        #         #     assert sep2 - sep1 == int(os.environ.get('TOTAL_COMPRESS_TOKENS', 0)), sep2 - sep1
        #         #     cls_embeddings = pooling(hidden_states[batch_idx:batch_idx+1, sep1:sep2, :])
        #         #     cls_embeddings_lst.append(cls_embeddings)

        #     if len(cls_embeddings_lst) > 0:
        #         cls_embeddings = torch.cat(cls_embeddings_lst, dim=0)
        #         cls_embeddings = cls_embeddings.unsqueeze(1).repeat(1, hidden_states.size(1), 1)
    ...
    def _update_causal_mask_by_sep_indices(
        self,
        ...
        # sep_indices: I,C1,C2,Q,A (I: image, C: compressed, Q: question, A: answer)
        if layer_idx == self.split_layer:
            new_causal_mask = causal_mask.clone()
            min_type = torch.finfo(dtype).min
            for batch_idx in range(len(new_causal_mask)):
                sep1, sep2 = sep_indices[batch_idx][:2]
                # I is ignored, C2<-Q<-A
                new_causal_mask[batch_idx, :, sep1:, :sep1] = min_type
            return new_causal_mask
        elif self.split_layer > 0 and layer_idx == 0:
            new_causal_mask = causal_mask.clone()
            min_type = torch.finfo(dtype).min
            for batch_idx in range(len(new_causal_mask)):
                sep1, sep2, sep3 = sep_indices[batch_idx][:3]
                # I<-C1<-C2, C2<-Q<-A
                new_causal_mask[batch_idx, :, sep3:, :sep2] = min_type
            return new_causal_mask
        return causal_mask
    ...
    def _update_causal_mask(
        ...
        assert self.config._attn_implementation == "sdpa", "For now, only SDPA supports our LLM causal mask updates."
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        ref_labels: Optional[torch.LongTensor] = None,
        ...
        *args, **kwargs
        ...
        if labels is not None:
            ...
            if ref_logits is not None:
                ...
                valid_logits, valid_ref_logits, loss_masks = [], [], []
                # 找到第一个A，也就是shift_ref_labels第一个为 True 的地方
                for shift_logit, shift_label, shift_ref_logit, shift_ref_label in zip(shift_logits, shift_labels, shift_ref_logits, shift_ref_labels):
                    answer_start = torch.where(shift_label != -100)[0][0]
                    ref_answer_start = torch.where(shift_ref_label != -100)[0][0]
                    # ref_labels 和 labels 应该后缀QA部分都是一样的
                    # assert shift_label[answer_start:].tolist() == shift_ref_label[ref_answer_start:].tolist()

                    shift_logit = shift_logit[answer_start:]
                    shift_ref_logit = shift_ref_logit[ref_answer_start:]

                    valid_logits.append(shift_logit)
                    valid_ref_logits.append(shift_ref_logit)

                    loss_mask = (shift_label[answer_start:] != -100).float()
                    loss_masks.append(loss_mask)

                # (total_batch_seq_len, vocab_size)
                valid_logits = torch.cat(valid_logits, dim=0)
                valid_ref_logits = torch.cat(valid_ref_logits, dim=0)
                valid_logits = F.log_softmax(valid_logits, dim=-1)
                valid_ref_logits = F.log_softmax(valid_ref_logits, dim=-1)

                # (batch_seq_len, )
                kl_loss = loss_fct(valid_logits, valid_ref_logits).sum(-1)
