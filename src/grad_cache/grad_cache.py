...
class GradCache:
    ...
    def build_cache(self, *reps: Tensor, deepspeed=None, **loss_kwargs) -> Union[List[Tensor], Tensor]:
        """
    ...
    def forward_backward(
        ...
        for i, (x, state, gradient, sync_context) in enumerate(zip(model_inputs, random_states, cached_gradients, sync_contexts)):
            with sync_context():
                with state:
                    y = self.model_call(model, x)
    ...
    def cache_step(
        ...
        **loss_kwargs
        ...
        for model, x in zip(self.models, model_inputs):
            model_reps, rnd_states = self.forward_no_grad(model, x)  # forward with sub-batches (batch_size // chunk_size) and concat to one resp (no grad)
            all_reps.append(model_reps)
            all_rnd_states.append(rnd_states)

        cache, loss = self.build_cache(*all_reps, deepspeed=deepspeed, **loss_kwargs)  # resp gradient (∂L/∂y)
        ...
        for model, x, model_cache, rnd_states in zip(
            ...
            self.forward_backward(model, x, model_cache, rnd_states, no_sync_except_last=no_sync_except_last, deepspeed=deepspeed)  # forward with sub-batches (batch_size // chunk_size) and backward with grad cache (∂L/∂y)
        return loss
