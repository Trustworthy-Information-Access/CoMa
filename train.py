...
from src.data.loader import init_openqa_dataloader, init_retrieval_dataloader
from src.model.model import MMEBModel
from src.trainer import GradCacheLateProcessTrainer, OpenQATrainer, MMEBTrainer
...
def main():
    ...
    print_rank(f'model: {model}')

    train_collator = MultimodalDataCollator(processor, model_args, data_args, training_args)

    all_dataset_config = {}
    for dataset_config_path in data_args.dataset_config:
        with open(dataset_config_path, 'r') as yaml_file:
            dataset_config = yaml.safe_load(yaml_file)
        print_rank(f"Loaded dataset config: {dataset_config_path}")
        for dataset_name, config in dataset_config.items():
            print_rank(f"\t{dataset_name}")
        all_dataset_config |= dataset_config

    if data_args.task_type == 'openqa':
        trainer_cls = OpenQATrainer
        train_dataloader = init_openqa_dataloader(all_dataset_config, model_args, data_args, training_args, train_collator)
    elif data_args.task_type == 'retrieval':
        train_dataloader = init_retrieval_dataloader(all_dataset_config, model_args, data_args, training_args, train_collator)
